...
export function AnalyticsCard({ item }: { item: AnalyticsSummaryCard }) {
  ...
  return (
    <Card className="group p-5 transition-all duration-base hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-sm font-medium text-muted-foreground">{item.title}</p>
          <p className="mt-3 break-words text-2xl font-bold">{item.value}</p>
        </div>
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-md bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
          <Icon className="h-5 w-5" aria-hidden="true" />
        </div>
      </div>
      <div
        className={cn(
          "mt-5 inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold",
        ...
        aria-label={`${item.title} trend: ${item.trend}`}
      >
